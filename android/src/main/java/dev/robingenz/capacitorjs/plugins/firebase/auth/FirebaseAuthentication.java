...
public class FirebaseAuthentication {
    ...
    public void handleSuccessfulSignIn(final PluginCall call, AuthCredential credential, String idToken, String id) {
        handleSuccessfulSignIn(call, credential, idToken, null, id);
    }

    public void handleSuccessfulSignIn(final PluginCall call, AuthCredential credential, String idToken, String nonce, String id) {
        boolean skipNativeAuth = this.config.getSkipNativeAuth();
        if (skipNativeAuth) {
            JSObject signInResult = FirebaseAuthenticationHelper.createSignInResult(null, credential, idToken, nonce, id);
            call.resolve(signInResult);
            return;
        }
        firebaseAuthInstance
                ...
                new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            FirebaseUser user = getCurrentUser();
                            JSObject signInResult = FirebaseAuthenticationHelper.createSignInResult(user, credential, idToken, nonce, id);
                            call.resolve(signInResult);
                        } else {
